...
define(['dojo', 'dojo/_base/declare', 'ebg/core/gamegui', 'ebg/counter', 'ebg/stock'], function (dojo, declare) {
	return declare('bgagame.justdessertssm', ebg.core.gamegui, {
		...
		onDraw: function (evt) {
			...
			if (this.checkAction('draw')) {
				this.ajaxcall('/justdessertssm/justdessertssm/drawAction.html', { lock: true }, this, function (result) {
					// animate
				})
			}
		},
		...
		onServeGuest: function (evt) {
			...
			if (selectedDesserts.length > 0 && selectedGuests.length == 1) {
				if (this.checkAction('serve')) {
					this.ajaxcall(
						'/justdessertssm/justdessertssm/serveAction.html',
						{
							lock: true,
		...
		onServeSecondGuest: function (evt) {
			...
			if (selectedDesserts.length > 0 && selectedGuests.length == 1) {
				if (this.checkAction('serveSecondGuest')) {
					this.ajaxcall(
						'/justdessertssm/justdessertssm/serveSecondGuestAction.html',
						{
							lock: true,
							cards_id: selectedDesserts.map((i) => i.id).join(';'),
							guest_id: selectedGuests[0].id
		...
		onDiscardGuest: function (evt) {
			...
			if (selectedGuests.length == 1) {
				if (this.checkAction('discardGuest')) {
					this.ajaxcall(
						'/justdessertssm/justdessertssm/discardGuestAction.html',
						{
							lock: true,
							guest_id: selectedGuests[0].id
						},
		...
		onOpenBuffet: function (evt) {
			...
			if (selectedDesserts.length == 4) {
				this.ajaxcall(
					'/justdessertssm/justdessertssm/openBuffetAction.html',
					{
						lock: true,
						cards_id: selectedDesserts.map((i) => i.id).join(';')
					},
		...
		onDiscardWonGuest: function (evt) {
			...
			if (selectedGuests.length == 1) {
				this.ajaxcall(
					'/justdessertssm/justdessertssm/discardWonGuestAction.html',
					{
						lock: true,
						guest_id: selectedGuests[0].id
					},
		...
		onPoach: function (evt) {
			...
			if (playersWithSelection.length == 1 && this.wonStocksByPlayerId[playersWithSelection[0]].getSelectedItems().length == 1) {
				if (selectedDesserts.length > 0) {
					...
					this.ajaxcall(
						'/justdessertssm/justdessertssm/poachAction.html',
						{
							lock: true,
		...
		onLetPoaching: function (evt) {
			...
			this.ajaxcall(
				'/justdessertssm/justdessertssm/letPoachingAction.html',
				{
					lock: true
				},
				this,
		...
		onBlockPoaching: function (evt) {
			...
			if (selectedDesserts.length > 0) {
				if (this.checkAction('blockPoaching')) {
					this.ajaxcall(
						'/justdessertssm/justdessertssm/blockPoachingAction.html',
						{
							lock: true,
							desserts_ids: selectedDesserts.map((i) => i.id).join(';')
						},
