	...
	return declare('bgagame.justdesserts', ebg.core.gamegui, {
		...
		setup: function (gamedatas) {
			...
			for (var player_id in gamedatas.players) {
				...
				dojo.place(
					this.format_block('jstpl_won_cards_icons', {
						id: player_id
					}),
					player_board_div
				)
				...
				for (var colorIndex in gamedatas.usefulColors) {
					dojo.place(
						this.format_block('jstpl_won_cards_icon', {
							id: player_id,
				...
				dojo.place(
					this.format_block('jstpl_cards_icon', {
						id: player_id
					}),
					player_board_div
				)
		...
		addCardToolTip: function (cards, card_id, card_type_arg, delay = 200) {
			...
			this.addTooltipHtml(
				curDiv,
				this.format_block('jstpl_card_tooltip_' + favouriteNumber + '_favorite', {
					backpos: backPos,
		...
		onDraw: function (evt) {
			...
			if (this.checkAction('draw')) {
				this.ajaxcall('/justdesserts/justdesserts/drawAction.html', { lock: true }, this, function (result) {
					// animate
				})
			}
		},
		...
		onExchange: function (evt) {
			...
			if (items.length > 0 || this.isSoloMode()) {
				if (this.checkAction('swap')) {
					this.ajaxcall(
						'/justdesserts/justdesserts/swapAction.html',
						{
							lock: true,
		...
		onServeGuest: function (evt) {
			...
			if (selectedDesserts.length > 0 && selectedGuests.length == 1) {
				if (this.checkAction('serve')) {
					this.ajaxcall(
						'/justdesserts/justdesserts/serveAction.html',
						{
							lock: true,
		...
		onServeSecondGuest: function (evt) {
			...
			if (selectedDesserts.length > 0 && selectedGuests.length == 1) {
				if (this.checkAction('serveSecondGuest')) {
					this.ajaxcall(
						'/justdesserts/justdesserts/serveSecondGuestAction.html',
						{
							lock: true,
							cards_id: selectedDesserts.map((i) => i.id).join(';'),
							guest_id: selectedGuests[0].id
		...
		onDiscardGuest: function (evt) {
			...
			if (selectedGuests.length == 1) {
				if (this.checkAction('discardGuest')) {
					this.ajaxcall(
						'/justdesserts/justdesserts/discardGuestAction.html',
						{
							lock: true,
							guest_id: selectedGuests[0].id
						},
		...
		onPass: function (evt) {
			...
			if (this.checkAction('pass')) {
				this.ajaxcall(
					'/justdesserts/justdesserts/passAction.html',
					{
						lock: true
					},
					this,
		...
		onEndSoloGame: function (evt) {
			...
			if (this.checkAction('endSoloGame')) {
				this.ajaxcall(
					'/justdesserts/justdesserts/endSoloGameAction.html',
					{
						lock: true
					},
					this,
		...
		onOpenBuffet: function (evt) {
			...
			if (selectedDesserts.length == 4) {
				this.ajaxcall(
					'/justdesserts/justdesserts/openBuffetAction.html',
					{
						lock: true,
						cards_id: selectedDesserts.map((i) => i.id).join(';')
					},
		...
		onDiscardWonGuest: function (evt) {
			...
			if (selectedGuests.length == 1) {
				this.ajaxcall(
					'/justdesserts/justdesserts/discardWonGuestAction.html',
					{
						lock: true,
						guest_id: selectedGuests[0].id
					},
					this,
					function (result) {
						this.wonStocksByPlayerId[this.player_id].removeFromStockById(selectedGuests[0].id)
					}
				)
			} else {
		...
		onPoach: function (evt) {
			...
			var selectedDesserts = this.playerHand.getSelectedItems()
			var playersWithSelection = Object.entries(this.wonStocksByPlayerId)
				.filter(([playerId, ws]) => ws.getSelectedItems().length > 0)
				.map((item) => item[0])
			...
			if (playersWithSelection.length == 1 && this.wonStocksByPlayerId[playersWithSelection[0]].getSelectedItems().length == 1) {
				if (selectedDesserts.length > 0) {
					...
					this.ajaxcall(
						'/justdesserts/justdesserts/poachAction.html',
						{
							lock: true,
		...
		onLetPoaching: function (evt) {
			...
			this.ajaxcall(
				'/justdesserts/justdesserts/letPoachingAction.html',
				{
					lock: true
				},
				this,
		...
		onBlockPoaching: function (evt) {
			...
			if (selectedDesserts.length > 0) {
				if (this.checkAction('blockPoaching')) {
					this.ajaxcall(
						'/justdesserts/justdesserts/blockPoachingAction.html',
						{
							lock: true,
							desserts_ids: selectedDesserts.map((i) => i.id).join(';')
						},
		...
		notif_newHand: function (notif) {
			...
			for (var i in notif.args.cards) {
				...
				if (notif.args.fromDiscard) {
					//given back after a blocked poach
